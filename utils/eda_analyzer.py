...
def plot_scatter(df, x_col, y_col):
    ...
    try:
        ...
        fig = px.scatter(
            ...
            color_discrete_sequence=COLOR_PALETTE,
            # WEBGL FIX: px.scatter silently switches to a WebGL-based
            # Scattergl trace once a dataset crosses Plotly's internal row
            # threshold (~1000 points as of Plotly 5.15+/6.x). On a browser
            # or environment without WebGL support, that trace fails to
            # render and shows "WebGL is not supported by your browser."
            # Forcing render_mode="svg" makes every scatter plot always use
            # the plain SVG renderer regardless of dataset size, so it works
            # everywhere, with identical zoom/hover/legend interactivity.
            render_mode="svg",
        )
