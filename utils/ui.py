...
def premium_table(
    ...
    white surface, rounded corners, soft shadow, thin borders, a solid
    opaque green header, zebra striping, row hover highlight, a sticky
    header, and a responsive/scrollable container.
    ...
    stylable, while still offering scrolling, a sticky header, and CSV
    download.

    NOTE: this used to also offer click-to-sort columns via an inline
    <script> tag appended after the table markup. That was removed —
    Streamlit's st.markdown(unsafe_allow_html=True) does not reliably
    execute injected <script> tags, and the leftover JS source (including
    its trailing "})();") was rendering as literal, visible text on the
    page instead of running. Rather than papering over that with CSS to
    hide the stray text, the actual cause (relying on a <script> tag inside
    st.markdown) has been removed. The table is now pure static HTML/CSS.
    ...
    header_cells = "".join(
        f'<th><span class="pp-th-label" title="{html.escape(col)}">'
        f'{html.escape(col) if col else "&nbsp;"}</span></th>'
        for col in columns
    )
...
def column_info_cards(df, dtypes: dict = None, missing_summary: dict = None):
    """
    Render a responsive grid of per-column info cards: Column Name, Data
    Type, Category (Numeric/Categorical), Non-Null count, Missing count,
    and Unique Values — replacing a plain "col_name dtype" text dump with
    a proper structured layout.

    Pure presentation: only reads df/dtypes/missing_summary to build
    display markup, never mutates them or touches session state.

    Args:
        df: pandas DataFrame the columns belong to (used for non-null /
            unique-value counts).
        dtypes: optional dict of {column: dtype string}. Recomputed from
            df if not supplied.
        missing_summary: optional dict of {column: missing count}.
            Recomputed from df if not supplied.
    """
    if df is None or len(df.columns) == 0:
        return
    ...
    for col in df.columns:
        series = df[col]
        dtype_str = str(dtypes.get(col)) if dtypes and col in dtypes else str(series.dtype)
        is_numeric = pd.api.types.is_numeric_dtype(series)
        category = "Numeric" if is_numeric else "Categorical"
        missing = int(missing_summary.get(col, series.isna().sum())) if missing_summary else int(series.isna().sum())
        non_null = total_rows - missing
        unique = int(series.nunique(dropna=True))
        safe_col = html.escape(str(col))

        # IMPORTANT: each card fragment is dedented/stripped individually
        # (via _clean()) BEFORE being joined below. Root cause of a prior
        # rendering bug: joining raw, still-indented multi-line triple-
        # quoted strings back-to-back left a whitespace-only line between
        # every two cards. Streamlit's Markdown parser treats a raw HTML
        # block as ending at the first blank (or whitespace-only) line —
        # so only the first card was ever parsed as real HTML; everything
        # after the first blank line reverted to plain Markdown, where the
        # heavily-indented remaining <div> markup was then classified as
        # an *indented code block* and printed as literal, visible text.
        # Cleaning each fragment first means the joined result has zero
        # blank lines anywhere in the whole block, so it can never be
        # split mid-stream like that again.
        cards.append(
            _clean(
                f"""
                <div class="pp-colinfo-card">
                    <div class="pp-colinfo-name" title="{safe_col}">{safe_col}</div>
                    <div class="pp-colinfo-row"><span>Data Type</span><strong>{html.escape(dtype_str)}</strong></div>
                    <div class="pp-colinfo-row"><span>Category</span><strong class="pp-colinfo-{category.lower()}">{category}</strong></div>
                    <div class="pp-colinfo-row"><span>Non-Null</span><strong>{non_null:,}</strong></div>
                    <div class="pp-colinfo-row"><span>Missing</span><strong>{missing:,}</strong></div>
                    <div class="pp-colinfo-row"><span>Unique Values</span><strong>{unique:,}</strong></div>
                </div>
                """
            )
        )
    ...
    st.markdown(
        f'<div class="pp-colinfo-grid">{grid_html}</div>',
        unsafe_allow_html=True,
    )
